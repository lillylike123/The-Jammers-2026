...
func _ready() -> void:
	...
	if hitbox: 
		...
		hitbox.monitorable = false  # Boss cannot detect itself
	if hurtbox: 
		hurtbox.monitoring = true
		hurtbox.monitorable = true  # Can be detected by player hitbox
	
	# Connect signals safely
	if hitbox and not hitbox.area_entered.is_connected(_on_hitbox_area_entered):
		hitbox.area_entered.connect(_on_hitbox_area_entered)  # YOU WERE MISSING THIS
